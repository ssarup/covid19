...
if __name__ == '__main__':
    ...
    if args.globalInputfile is not None:
        rdr = FileReader(args.globalInputfile)
        txg = Covid19GlobalTransformer()
        rdr.read(txg)
        print("Read {0} lines from input file.".format(len(txg.getCollection())))
        for covid19Obj in txg.getCollection():
            Covid19Utils.addCSVToObsCollection(covid19Obj, coll)
